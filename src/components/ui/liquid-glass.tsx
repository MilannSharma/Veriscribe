...
const GlassEffect: React.FC<GlassEffectProps> = ({
  ...
  const content = (
    <div
      ...
      className={`relative flex font-semibold overflow-hidden text-black transition-all duration-700 ${className} ${!disabled && onClick ? 'cursor-pointer' : ''}`}
      style={glassStyle}
    >
      {/* Glass Layers */}
      <div
    ...
    <div className="flex items-center justify-center gap-2 rounded-3xl p-3 py-0 px-0.5 overflow-hidden">
      {icons.map((icon, index) => (
        <img
          ...
          src={icon.src}
          alt={icon.alt}
          className="w-16 h-16 transition-all duration-700 hover:scale-110"
          style={{
